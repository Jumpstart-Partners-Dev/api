...
class StoreController extends Controller
{
    ...
    public function admin_getStores(Request $req) {
        $where = "status = 'published'";

        // foreach(explode(" ", $req->filter) as $keys) {
        //     $where .= " and alias like '%" . $keys. "%' ";      
        // }

        $where .= " and alias like '%" .str_replace(' ', '-', $req->filter). "%' ";  

        $stores = DB::table('stores')
            ->select('id', 'name', 'logo', 'alias', 'affiliate_url')
            ->whereRaw($where)
            ->orderBy('id', 'asc')
            ->paginate(20);
        ...
        $total = 0;
        if ($req->page == 1) {
            $total = DB::table('stores')->where('status', 'published')->count();
        }

        $commission = 0;
        if ($req->page == 1) {
            $commission = DB::table('store_commissions')
                ->selectRaw('coalesce(sum(unpaid), 0) as unpaid')
                ->first();
        }

        echo json_encode(['data' => $stores, 'total' => $total, '', 'commissions' => $commission->unpaid]);
    }

    public function admin_getStore($id) {
        ...
        echo json_encode(['store' => $store, 'commission' => $commission]);
    }
